...
def train(EPOCHS, model, train_loader, input_data_time, output_data_time):
    ...
    for epoch in range(EPOCHS):
        ...
        loss_test = 0.0
        for j, cic_train in enumerate(train_loader):
            input_data_time.to(device)
            model = model.to(device)
            test = model(input_data_time[0][0].unsqueeze(0).unsqueeze(0))
...
def main():
    ...
    output = output.reshape(-1,).detach().numpy()
    #print("test_x=",test_x)
    plt.plot(output, linestyle="None", linewidth=0, marker='o')
    plt.show()
